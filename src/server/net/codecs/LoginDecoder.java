...
public class LoginDecoder extends FrameDecoder {
	...
	protected Object decode(ChannelHandlerContext ctx, Channel channel, ChannelBuffer in) throws Exception {
		...
		case LOGGING_IN:
			...
			String username = ScimitarUtility.getRS2String(in);
			String password = ScimitarUtility.getRS2String(in);
	...
	private static ScimitarPlayerIO login(Channel channel, ISAACCipher inCipher, ISAACCipher outCipher, int version, String name, String pass) {
			...
			ScimitarPlayerIO client = new ScimitarPlayerIO(channel);
