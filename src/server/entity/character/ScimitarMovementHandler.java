 ...
 * @author blakeman8192
 */
public class ScimitarMovementHandler implements MovementHandler {
	 ...
	 *            the Player
	 */
	public ScimitarMovementHandler(CharacterEntity entity) {
		this.entity = entity;
	}
	...
	public void tick() {
		...
		if (walkPoint != null && walkPoint.getDirection() != -1) {
			entity.getPosition().move(ScimitarUtility.DIRECTION_DELTA_X[walkPoint.getDirection()],
					ScimitarUtility.DIRECTION_DELTA_Y[walkPoint.getDirection()]);
			entity.setPrimaryDirection(walkPoint.getDirection());
		}
		if (runPoint != null && runPoint.getDirection() != -1) {
			entity.getPosition().move(ScimitarUtility.DIRECTION_DELTA_X[runPoint.getDirection()],
					ScimitarUtility.DIRECTION_DELTA_Y[runPoint.getDirection()]);
		...
		if (deltaX < 16 || deltaX >= 88 || deltaY < 16 || deltaY > 88) {
			if (!(entity instanceof NPC)) {
				((ScimitarPlayerIO) ((Player) entity).getIO()).sendMapRegion();
			}
		}
	}
	...
	private void addStep(int x, int y) {
		...
		int direction = ScimitarUtility.direction(deltaX, deltaY);
		if (direction > -1) {
